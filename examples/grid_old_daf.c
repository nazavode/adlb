/* This older version of the program does not agree with grid_uni 
 * in terms of computed result.  This one does NOT make sure that the 
 * latest neighbor rows are on either side of a row to be updated.  To do
 * that, it would need to make sure that each row stays in lock step with
 * its neighbors, which requires more sync and slows things down a bit by
 * mnaking rank 0 something of a bottleneck.  The newer version of this
 * program does that.
 */
...
void work(int my_rank)
{
    int i, rc, done, work_type, work_prio, work_handle[ADLB_HANDLE_SIZE], work_len,
        answer_rank, req_types[4], num_rows_done = 0;
