/*
    This version of adlb.c is an alternative of version 425 which 
    was altered to fix a problem with IBM's MPI on the Mira system
    at Argonne.  In particular, that MPI would not hndle NULL as
    the pointer to a message when the size of the message is 0 bytes.
    Also, it did not deal well with 0-byte messages (tag is the entire
    payload).  So, Jeff Hammond added use of a buffer and caused one
    element to be sent instead of 0.
*/
...
static char *svn_revision = "$Revision: 423 $";
static char *svn_date = "$LastChangedDate: 2013-01-28 11:08:17 -0600 (Mon, 28 Jan 2013) $";
            ...
            (MPI_Wtime()-prev_exhaust_chk_time) > exhaust_chk_interval)
        {
            if (rq->count >= num_apps_this_server)
            {
                ...
                else
                {
                    ...
                    temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                    MPI_Isend(info_buf_jeff,1,MPI_INT,rhs_rank,SS_EXHAUST_CHK_LOOP_1,adlb_all_comm,temp_req);
#else
                    MPI_Isend(info_buf,0,MPI_INT,rhs_rank,SS_EXHAUST_CHK_LOOP_1,adlb_all_comm,temp_req);
#endif
...
#       else
        ...
        else if (from_tag == FA_NO_MORE_WORK)
        {
...
#ifdef JEFF
            MPI_Recv(info_buf_jeff,1,MPI_INT,from_rank,from_tag,adlb_all_comm,&status);
#else
            ...
            if (my_world_rank == master_server_rank)
            {
                if (num_servers > 1)
                {
                    temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                    rc = MPI_Issend(info_buf_jeff,1,MPI_INT,rhs_rank, SS_NO_MORE_WORK,adlb_all_comm,temp_req);
#else
                    rc = MPI_Issend(info_buf,0,MPI_INT,rhs_rank, SS_NO_MORE_WORK,adlb_all_comm,temp_req);
#endif
                    iq_node = iq_node_create(temp_req,0,NULL);
                    iq_append(iq_node);
                }
            ...
            else
            {
                ...
                temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                rc = MPI_Issend(info_buf_jeff,1,MPI_INT,master_server_rank, SS_NO_MORE_WORK,adlb_all_comm,temp_req);
#else
                rc = MPI_Issend(info_buf,0,MPI_INT,master_server_rank, SS_NO_MORE_WORK,adlb_all_comm,temp_req);
#endif
                iq_node = iq_node_create(temp_req,0,NULL);
                iq_append(iq_node);
            }
        ...
        else if (from_tag == SS_END_LOOP_1)
        {
            ...
            if (my_world_rank == master_server_rank)
            {
                ...
                temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                rc = MPI_Issend(info_buf_jeff,1,MPI_INT,rhs_rank, SS_END_LOOP_2,adlb_all_comm,temp_req);
#else
                rc = MPI_Issend(info_buf,0,MPI_INT,rhs_rank, SS_END_LOOP_2,adlb_all_comm,temp_req);
#endif
                iq_node = iq_node_create(temp_req,0,NULL);
                iq_append(iq_node);
            }
            else
            {
                if (num_local_apps_done >= num_apps_this_server)
                {
                    ...
                    temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                    rc = MPI_Issend(info_buf_jeff,1,MPI_INT,rhs_rank, SS_END_LOOP_1,adlb_all_comm,temp_req);
#else
                    rc = MPI_Issend(info_buf,0,MPI_INT,rhs_rank, SS_END_LOOP_1,adlb_all_comm,temp_req);
#endif
                    iq_node = iq_node_create(temp_req,0,NULL);
                    iq_append(iq_node);
                }
        ...
        else if (from_tag == SS_END_LOOP_2)
        {
            ...
            {
#ifdef JEFF
                rc = MPI_Issend(info_buf_jeff,1,MPI_INT,debug_server_rank, DS_END,adlb_all_comm,&dummy_req);
#else
                rc = MPI_Issend(info_buf,0,MPI_INT,debug_server_rank, DS_END,adlb_all_comm,&dummy_req);
#endif
            }
            if (my_world_rank != master_server_rank)
            {
        ...
        else if (from_tag == SS_EXHAUST_CHK_LOOP_1)
        {
            ...
            if (my_world_rank == master_server_rank)
            {
                if (rq->count >= num_apps_this_server  &&  exhausted_flag) /* be sure */
                {
                    temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                    MPI_Isend(info_buf_jeff,1,MPI_INT,rhs_rank,SS_EXHAUST_CHK_LOOP_2, adlb_all_comm,temp_req);
#else
                    MPI_Isend(info_buf,0,MPI_INT,rhs_rank,SS_EXHAUST_CHK_LOOP_2, adlb_all_comm,temp_req);
#endif
                    iq_node = iq_node_create(temp_req, 0, NULL);
                }
            }
        ...
        else if (from_tag == SS_EXHAUST_CHK_LOOP_2)
        {
            ...
            if (rq->count >= num_apps_this_server  &&  exhausted_flag) /* be sure */
            {
                if (my_world_rank == master_server_rank)
                {
                    temp_req = amalloc(sizeof(MPI_Request));
#ifdef JEFF
                    MPI_Isend(info_buf_jeff,1,MPI_INT,rhs_rank,SS_DONE_BY_EXHAUSTION, adlb_all_comm,temp_req);
#else
                    MPI_Isend(info_buf,0,MPI_INT,rhs_rank,SS_DONE_BY_EXHAUSTION, adlb_all_comm,temp_req);
#endif
                    iq_node = iq_node_create(temp_req, 0, NULL);
                }
                else
...
#ifdef JEFF
            MPI_Recv(info_buf_jeff,1,MPI_INT,from_rank,FA_LOCAL_APP_DONE,adlb_all_comm,&status);
#else
...
{
#ifdef JEFF
    MPI_Ssend(&dummy,1,MPI_INT,my_server_rank,FA_NO_MORE_WORK,adlb_all_comm);
#else
    MPI_Ssend(&dummy,0,MPI_INT,my_server_rank,FA_NO_MORE_WORK,adlb_all_comm);
#endif
    return ADLB_SUCCESS;
}
...
int ADLBP_Finalize()
{
    ...
    {   
#ifdef JEFF
        rc = MPI_Ssend(&dummy,1,MPI_INT,my_server_rank,FA_LOCAL_APP_DONE,adlb_all_comm);
#else
